...
async function getStats(player, uuid, mode, time, username) {
  const [response, response24H] = await Promise.all([axios.get(`${config.api.hypixelAPI}/player?uuid=${uuid}&key=${config.api.hypixelAPIkey}`), axios.get(`${config.api.pixelAPI}/${time}?key=${config.api.pixelKey}&uuid=${uuid}`)])
  ...
  if (["gen", "general", "g"].includes(mode.toLowerCase())) {
    ...
    const bedwarsLevel = (getBedwarsLevel(bedwarsData.Experience) - oldBedwarsData.levelRaw).toFixed(3)
    ...
    return `/gc [${bedwarsLevel}✫] ${player} FK: ${addCommas(bedwarsFinalKills)} FKDR: ${bedwarsFkdr} | Wins: ${bedwarsWins} WLR: ${bedwarsWlr} | BB: ${bedwarsBedsBroken} BLR: ${bedwarsBblr} | in the last ${lastTime}`
    ...
    const skywarsLevel = (getSkywarsLevel(skywarsData.skywars_experience) - oldSkywarsData.levelRaw).toFixed(3)
