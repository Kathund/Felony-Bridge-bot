  ...
  if (check != 'disabled') {
    ...
    fetch(`https://api.hypixel.net/player?uuid=${UUID}&key=${KEY}`).then((res) => {res.json().then((player) => {
        ...
        if (mode == 'solo') {            
            ...
            this.send(`/gc Daily stats for ${PlayerName}: Wins: ${bedwars_wins} WLR: ${bedwars_wlr} FKDR: ${bedwars_fkdr} BLR: ${bedwars_bblr} Finals: ${bedwars_finalKills} Beds: ${bedwars_bedsBroken} - ${makeid(10)}`)
        }
