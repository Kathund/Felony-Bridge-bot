...
onCommand(username, message) {
  if (check != 'disabled') {
    if (username == 'help') {
        this.send(`/gc Usage: !weekly <username> - ${makeid(10)}`)
    }
    else {
    ...
    fetch(`https://api.pixelic.de/v1/player/weekly?uuid=${UUID}`).then((res) => {res.json().then((Daily) => { // For Weekly/Monthly just replace ".../daily?uuid=..." with ".../weekly?uuid=..." or ".../monthly?uuid=..." the rest of the code will still say daily but thats the easiest way and u are fricking lazy
    fetch(`https://api.hypixel.net/player?uuid=${UUID}&key=${KEY}`).then((res) => {res.json().then((player) => {
        ...
        if (mode == 'solo') {
            this.send(`/gc Weekly stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_solo_wins} FKDR: ${Bedwars_solo_fkdr} BLR: ${Bedwars_solo_bblr} Finals: ${Bedwars_solo_finalKills} Beds: ${Bedwars_solo_bedsBroken} - ${makeid(10)}`)
        }
        else if (mode == 'doubles') {
            this.send(`/gc Weekly stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_doubles_wins} FKDR: ${Bedwars_doubles_fkdr} BLR: ${Bedwars_doubles_bblr} Finals: ${Bedwars_doubles_finalKills} Beds: ${Bedwars_doubles_bedsBroken} - ${makeid(10)}`)
        }
        else if (mode == 'threes') {
            this.send(`/gc Weekly stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_threes_wins} FKDR: ${Bedwars_threes_fkdr} BLR: ${Bedwars_threes_bblr} Finals: ${Bedwars_threes_finalKills} Beds: ${Bedwars_threes_bedsBroken} - ${makeid(10)}`)
        }
        else if (mode == 'fours') {
            this.send(`/gc Weekly stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_fours_wins} FKDR: ${Bedwars_fours_fkdr} BLR: ${Bedwars_fours_bblr} Finals: ${Bedwars_fours_finalKills} Beds: ${Bedwars_fours_bedsBroken} - ${makeid(10)}`)
        }
        else if (mode == '4v4') {
            this.send(`/gc Weekly stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_four_two_wins} FKDR: ${Bedwars_four_two_fkdr} BLR: ${Bedwars_four_two_bblr} Finals: ${Bedwars_four_two_finalKills} Beds: ${Bedwars_four_two_bedsBroken} - ${makeid(10)}`)
        }
        else if (mode == 'overall') {
            this.send(`/gc Weekly stats for ${PlayerName}: Wins: ${bedwars_wins} FKDR: ${bedwars_fkdr} BLR: ${bedwars_bblr} Finals: ${bedwars_finalKills} Beds: ${bedwars_bedsBroken} - ${makeid(10)}`)
        }
        else {
            this.send(`/gc Weekly stats for ${PlayerName}: Wins: ${bedwars_wins} FKDR: ${bedwars_fkdr} BLR: ${bedwars_bblr} Finals: ${bedwars_finalKills} Beds: ${bedwars_bedsBroken} - ${makeid(10)}`)
        }
