...
onCommand(username, message) {
  if (check != 'disabled') {
    ...
    fetch(`https://api.hypixel.net/player?uuid=${UUID}&key=${KEY}`).then((res) => {res.json().then((player) => {
        ...
        if (mode == 'solo') {
            this.send(`/gc Daily stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_solo_wins} FKDR ${Bedwars_solo_fkdr} BLR ${Bedwars_solo_bblr} Finals ${Bedwars_solo_finalsKills} Beds ${Bedwars_solo_bedsBroken}`)
        }
        else if (mode == 'doubles') {
            this.send(`/gc Daily stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_doubles_wins} FKDR ${Bedwars_doubles_fkdr} BLR ${Bedwars_doubles_bblr} Finals ${Bedwars_doubles_finalsKills} Beds ${Bedwars_doubles_bedsBroken}`)
        }
        else if (mode == 'threes') {
            this.send(`/gc Daily stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_threes_wins} FKDR ${Bedwars_threes_fkdr} BLR ${Bedwars_threes_bblr} Finals ${Bedwars_threes_finalsKills} beds ${Bedwars_threes_bedsBroken}`)
        }
        else if (mode == 'fours') {
            this.send(`/gc Daily stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_fours_wins} FKDR ${Bedwars_fours_fkdr} BLR ${Bedwars_fours_bblr} Finals: ${Bedwars_fours_finalsKills} Beds: ${Bedwars_fours_bedsBroken}`)
        }
        else if (mode == '4v4') {
            this.send(`/gc Daily stats for ${PlayerName} in ${mode}: Wins: ${Bedwars_four_two_wins} FKDR ${Bedwars_four_two_fkdr} BLR ${Bedwars_four_two_bblr} Finals ${Bedwars_four_two_finalKills} Beds ${Bedwars_four_two_bedsBroken}`)
        }
        else if (mode == 'overall') {
            this.send(`/gc Daily stats for ${PlayerName}: Wins: ${Bedwars_wins} FKDR ${Bedwars_fkdr} BLR ${Bedwars_bblr} Finals ${Bedwars_finalKills} Beds ${Bedwars_bedsBroken}`)
        }
        else {
            this.send(`/gc Daily stats for ${PlayerName}: Wins: ${Bedwars_wins} FKDR ${Bedwars_fkdr} BLR ${Bedwars_bblr} Finals ${Bedwars_finalKills} Beds ${Bedwars_bedsBroken}`)
        }
