...
class GuildLeaderboardCommand extends minecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      if (["lifetime", "alltime", "all", "l"].includes(msg[0])) timeframe = "Lifetime"
      ...
      fetch(`https://api.pixelic.de/v1/leaderboard/guild?key=${config.api.pixelKey}&mode=${mode.toLowerCase()}&timeframe=${timeframe.toLowerCase()}&limit=10&uuid=${await getUUID(username)}`).then((res) => {
        res.json().then(async (data) => {
          ...
          if (mode.toLowerCase() == "bedwars") {
            //check gamemode
            if (msg[2] == "overall") gamemode = "overall", gamemodeFormatted = "Overall"
            if (msg[2] == "solo") gamemode = "solo", gamemodeFormatted = "Solo"
            if (msg[2] == "doubles") gamemode = "doubles", gamemodeFormatted = "Doubles"
            if (msg[2] == "threes") gamemode = "threes", gamemodeFormatted = "threes"
            if (msg[2] == "fours") gamemode = "fours", gamemodeFormatted = "Fours"
            if (["four_two", "4v4"].includes(msg[2])) gamemode = "four_two", gamemodeFormatted = "4v4"
            ...
            if (["finals", "finalkills", "fk", "fks", "fkill", "fkills"].includes(msg[3])) type = "finalKills", typeFormatted = "Finalkills"
            if (["finaldeaths", "fd", "fds", "fdeath", "fdeaths"].includes(msg[3])) type = "finalDeaths", typeFormatted = "Finaldeaths"
            ...
            if (["bedsbroken", "bb"].includes(msg[3])) type = "bedsBroken", typeFormatted = "BedsBroken"
            if (["bedslost", "bl"].includes(msg[3])) type = "bedsLost", typeFormatted = "BedsLost"
            ...
            if (msg[2] == "overall") gamemode = "overall", gamemodeFormatted = "Overall"
            if (msg[2] == "solo") gamemode = "solo", gamemodeFormatted = "Solo"
            if (["teams", "doubles"].includes(msg[2])) gamemode = "doubles", gamemodeFormatted = "Doubles"
            ...
            if (["quest", "quests"].includes(msg[2])) type = "quests", typeFormatted = "Quests"
